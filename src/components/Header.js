...
const Header = () => {
  ...
  return (
    <div className="absolute flex flex-col md:flex-row md:justify-between w-full z-10 px-8 py-2 bg-gradient-to-b from-black">
      ...
      {/* <div className="text-2xl font-bold text-gray-800 mx-auto md:mx-0"> */}
      <div
        className="text-4xl font-bold mx-auto md:mx-0"
        style={{ color: "#36fffc" }}
      >
        StreamGPT
      </div>
          ...
          {showGptSearch && (
            ...
            >
              {SUPPORTED_LANGUAGES.map((lang) => (
                <option key={lang.ideantifier} value={lang.ideantifier}>
                  {lang.name}
                </option>
              ))}
            </select>
